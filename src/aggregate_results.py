...
def main():
    ...
    ax.set_title("CarRacing-v3 — mean rolling return across all seeds")
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    ...
    ax.set_title(f"Per-episode runtime across all seeds (rolling w={dur_w})")
    ax.legend()
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    ...
    ax.set_title("Total runtime per variant (mean ± std across all seeds)")
